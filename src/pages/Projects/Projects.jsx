        ...
        <div className="w-full  grid  grid-cols-2  lg:grid-cols-3 xl:grid-cols-5 gap-1 p-1 bg-primaryColor">
          {data.map((item, index) => (
            <div
              ...
              className="w-full overflow-hidden relative  aspect-square  group bg-cover bg-center bg-no-repeat opacity-0  animate-[appearWithScale_1000ms_ease-in-out_forwards] "
            >
              <NavLink
                to={item.url}
                style={{ backgroundImage: `url(${item.featuredImage})` }}
              ...
              >
                <div className="absolute w-full h-full   ease-out ">
                  <img
                    src={item.coverImage}
                    className=" group-hover:opacity-0 duration-[350ms] ease-[cubic-bezier(.42,-0.01,.47,1)] w-full h-full "
                    alt=""
                  />
